export const siteText = {
  ...
  guarantee: {
    ...
    card1: {
      text: "Юридическая",
      description:
        "Вы приобретаете автомобиль без кредитных и залоговых обязательств, измененных маркировок, запрета на регистрацию и других ограничений.",
    },
    card2: {
      text: "Техническая",
      description:
        "Отремонтируем ваш автомобиль. Если неисправность не была выявлена при диагностике автомобиля.",
    },
    card3: {
      text: "100% Возврат денег",
      description:
        "Если мы не сможем найти для вас требуемый автомобиль в установленные сроки — вернем предоплату в полном объёме.",
    },
  },
  ...
  action: {
    ...
    description:
      "Найдем вам идеальное предложение на рынке, которое полностью соответствует вашим требованиям и обладает безупречной юридической и технической документацией",
  },
};

export const SERVICES_1TITLE = "Наши услуги";
export const SERVICES_TITLE = {
  first: "Подбор под ключ",
...
export const SERVICES_CARDS = {
  ...
  fourth: {
    ...
    price: "от 30.000рублей ",
    description: [
      "Анализ рынка и осмотр на подъемнике",
...
export const SERVICES_CARDS_MODAL = {
  first: {
    title: "Подбор под ключ",
    info: [
      "Вы не до конца определились с маркой и моделью авто",
      "Вы начинающий автолюбитель",
      "Не хватает времени на самостоятельный поиск авто",
      "Не желаете общаться с перекупщиками или салонами",
      "Не уверены, что грамотно сможете провести диагностику авто",
      "Необходимо купить автомобиль без залогов и ограничений",
      "Найти лучший авто в своем бюджете",
      "Регион, в котором вы находитесь, мало предложений",
    ],
  },
  second: {
    title: "Выездная диагностика",
    info: [
      "Если вы уже определились с выбором автомобиля",
      "Вы нашли автомобиль, который хотели, и нужно узнать все его дефекты",
      "Вы не уверены в честности автомобиля или продавца",
      "Вы не можете самостоятельно осмотреть автомобиль",
      "Вы находитесь в другом городе",
      "Не отпускает работа или учеба",
      "После осмотра эксперт подскажет состояние автомобиля, и вы примете решение",
      "Вы получите описание состояния и полный перечень неисправностей",
    ],
  },
  third: {
    title: "Эксперт на день",
    info: [
      "Вы уже нашли несколько интересных для вас авто",
      "Вы планируете осмотреть их все за один день",
      "Приехали из другого города и нужна помощь эксперта в осмотре",
      "У вас нет времени самостоятельно осмотреть авто",
      "Осмотры могут проводиться без вашего присутствия, все фото с осмотра предоставим",
      "Мы поможем добраться до найденных авто",
      "Перемещение по осмотрам на авто эксперта",
      "Вы получите описание состояния и полный перечень неисправностей",
    ],
  },
  fourth: {
    ...
    info: [
      ...
      "Не хватает времени на самостоятельный поиск авто",
      "Вы не можете самостоятельно осмотреть автомобиль",
      "Хотите подобрать автомобиль из текущих предложений на рынке",
      "Найти лучший авто в своем бюджете",
      "Вы получите описание состояния и полный перечень неисправностей",
      "Получить авто в кротчайшие сроки (2-3 дня)",
    ],
  ...
  fifth: {
    ...
    info: [
      ...
      "Вы не до конца определились с маркой и моделью авто",
      "Не желаете общаться с перекупщиками или салонами",
      "Не знаете, где «серый» салон, а где перекупщики",
    ],
  },
  sixth: {
    title: "Предпродажная оценка",
    info: [
      "Если вы решили продать свой старый автомобиль",
      "Не знаете его точную рыночную стоимость",
      "Не хотите продать слишком дешево, слить перекупщикам",
      "Проведем осмотр и вы получите фирменный бланк с рекомендациями и состоянием",
    ],
  },
};
...
export const HERO_INFO = {
  ...
  description:
    "Мы поможем вам найти идеальный подержанный автомобиль, который соответствует вашим потребностям и бюджету. Наш индивидуальный подход и прозрачный процесс гарантируют, что вы получите лучший автомобиль по лучшей цене.",
...
export const CHECK_CARDS = {
  card1: {
    text: "ДВС.",
    description:
      "Проверка состояния течи, запотевания, компрессии, эндоскопия, компьютерная диагностика.",
    icon: "./img/check/DVS.svg",
  },
  card2: {
    text: "Кузов.",
    description:
      "Проверка кузова на следы ремонта, перекраски, замены деталей.",
    icon: "./img/check/Body.svg",
  },
  card3: {
    text: "Рулевое управление.",
    description:
      "Проверка наличия люфтов, стуков, подтеков рабочей жидкости и равномерный ход.",
  ...
  card5: {
    text: "АКПП.",
    description:
      "Проверка правильности переключения передач, компьютерная диагностика, отсутствие течи.",
    icon: "./img/check/AKPP.svg",
  },
  card6: {
    text: "Юридическая чистота.",
    description:
      "Сверка маркировочных табличек и VIN номера, проверка по базам ГИБДД и Реестра залогов, подтверждение подлинности ПТС.",
    icon: "./img/check/Legal.svg",
  },
  card7: {
    text: "Тормоза.",
    description:
      "Проверка состояния дисков, колодок, тормозных механизмов и систем помощи при торможении.",
...
export const RISK_CARD = {
  card1: {
    text: "Деньгами",
    description:
      "– Крупные непредвиденные вложения после покупки которые вы могли не заметить",
  ...
  card3: {
    text: "Нервами",
    description:
      "– Позаботьтесь о своем спокойствие, все нудные переговоры и поиски оставьте специалистам",
